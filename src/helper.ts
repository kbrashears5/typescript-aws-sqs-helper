...
export class SQSHelper extends BaseClass implements ISQSHelper {
  ...
  public async CreateQueueAsync(
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(queueName)) {
      throw new Error(`[${action}]-Must supply queueName`);
    }
  ...
  public async DeleteMessageAsync(
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(receiptHandle)) {
      throw new Error(`[${action}]-Must supply receiptHandle`);
    }
  ...
  public async DeleteMessagesAsync(
    queueUrl: string,
    ...
    if (!receiptHandles || receiptHandles.length === 0) {
      throw new Error(`[${action}]-Must supply at least one receiptHandle`);
    }
    if (receiptHandles.length > 10) {
      throw new Error(`[${action}]-Can only supply up to 10 receiptHandles`);
    }
  ...
  public async GetNumberOfMessagesOnQueueAsync(
    queueUrl: string,
  ): Promise<number> {
  ...
  public async ReceiveAllMessagesAsync(
    queueUrl: string,
  ...
  public async ReceiveMessagesAsync(
    queueUrl: string,
  ...
  public async SendMessageAsync(
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(messageBody)) {
  ...
  public async SendMessagesAsync(
    queueUrl: string,
    ...
    if (!entries || entries.length === 0) {
      throw new Error(`[${action}]-Must supply at least one entry`);
    }
